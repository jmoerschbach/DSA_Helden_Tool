package steigerrechner;

/**
 * Created by Hauke on 12.07.2017.
 */
public class Steigerrechner {

    private static int[][] Kosten;

    public static void initializeKostenTabelle() {
    ...
    public static int getCost(int von, int auf, int spalte) {
        int vonCost = getSum(spalte, von);
        int aufCost = getSum(spalte, auf);
        return aufCost - vonCost;
    }

    public static int getCost(int von, int auf, String spalte) {
        int s;
        switch (spalte) {
            case "A":   s = 1;
                break;
    ...
    private static int getValue(int spalte, int zeile) {
        return Kosten[spalte][zeile];
    }
}
